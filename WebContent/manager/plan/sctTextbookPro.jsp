...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<form method="post" name="form1"
			action="<%=request.getContextPath()%>/plan/syllabus_getTextBooks.do">
			<div class="tjcx">
				<dl>
					<dt>
						出版时间：
					...
					<tbody>
						<c:forEach items="${ textbookList}" var="textbook"
							...
							<tr>
								...
								<td align="center">
									<a href="#"
										onclick='$("#textbookName",window.opener.document).val("${ textbook.textbookName}");$("#textbookCode",window.opener.document).val("${ textbook.textbookCode}");<c:if test="${ textbook.unifiedBook eq '1'}">$("#textbookUnitary1",window.opener.document).attr("checked", "checked");$("#textbookUnitary0",window.opener.document).removeAttr("checked");</c:if><c:if test="${ textbook.unifiedBook eq '0'}">$("#textbookUnitary0",window.opener.document).attr("checked", "checked");$("#textbookUnitary1",window.opener.document).removeAttr("checked");</c:if>window.close();'>${

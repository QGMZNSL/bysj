...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<!--导航---end-->
		<!--编辑页面----开始-->
		<form action="information_doedit.do" method="post" onsubmit="return check()" enctype="multipart/form-data">
		<div class="infoedit">
			
				<input type="hidden" name="information.informationId" value="${information.informationId}"/>
				<dl>
					<dt>
						标题：
					</dt>
					<dd>
					<input class="inputText" type="text" name="information.informationTitle" id="title" value="${information.informationTitle}"/>
					<span>(必须填写)</span>
					</dd>
				</dl>
				<dl>
						...
						<select class="inputText" name="information.classId" id="classid">
							<c:forEach var="class" items="${infoClassList}">
								<option value="${class.classId }"
									<c:if test="${information.classId==class.classId }">selected</c:if>>
									${class.className }
								</option>
							</c:forEach>
						</select>
				...
				<dl>
					...
					<dd>
						<textarea class="inputTextarea" name="information.informationMainContent"
							id="content">${information.informationMainContent}</textarea>
					</dd>
				</dl>
				<dl>
					<dt>
					...
					<dd>
						<c:if
							test="${not empty information.informationFile}">
							<input type="text" value="${information.informationFile }" readonly />
						</c:if>
		...
		<div class="button">
			...
			<input class="inputButton" type="button" value='重 置' onclick="location.href='information_toedit.do?inforid=${information.informationId}'"/>

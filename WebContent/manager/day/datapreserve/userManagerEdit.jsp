...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function init() {
			...
			// 初始化地市
			$('#cityCode').val('${ cuser.cityCode}');
			
			ajaxFitSelect(usrRole);
			
			$('#positionCode').val('${ cuser.positionCode}');
			$('#academyCode').val('${ cuser.academyCode}');
			$('#cameraPlaceCode').val('${ cuser.cameraPlaceCode}');
			$('#unitCode').val('${ cuser.unitCode}');
	...
	<body>
		...
		<!--编辑页面----开始-->
		<form
			action="<%=request.getContextPath()%>/day/datapreserve/usrMgr_saveUser.do?userSaveMethod=forOld"
			...
			<v:bean clazz="com.sincinfo.zxks.core.day.datapreserve.UserMgrAction"
				...
				<div class="infoedit">
					<dl>
						...
						<dd>
							<input class="inputText inputTextM" type="text" readonly="readonly"
								name="cuser.userName" id="userName" value="${ cuser.userName}" />
							<span>(必须填写)</span>
							<v:v input="cuser.userName" exp="ename(4,20)">请输入登录名！</v:v>
						</dd>
					</dl>
					<dl>
						<dt>
						...
						<dd>
							<input class="inputText inputTextM" type="text"
								name="cuser.realName" value="${ cuser.realName}" />
							<span>(必须填写)</span>
						...
						<dd>
							<c:forEach items="${ cuser.genderList}" var="gender">
								<input type="radio" name="cuser.userGender" class="userGender"
									<c:if test="${ gender.code eq cuser.userGender}">checked="checked"</c:if>
					...
					<dl>
						...
						<dd>
							...
							<v:v input="cuser.userPassword" exp="_null&(s(0,0)|password(6,16))">密码填写不正确，请重新填写！</v:v>
						</dd>
					</dl>
					<dl>
						<dt>
					...
					<dl>
						...
						<dd>
							<input class="inputText inputTextM" type="text"
								name="cuser.telephone" value="${ cuser.telephone}" />
							<span>(必须填写)</span>
					...
					<dl>
						...
						<dd>
							<input class="inputText inputTextM" type="text"
								name="cuser.postalAddress" value="${ cuser.postalAddress}" />
							<span>(必须填写)</span>
					...
					<dl>
						...
						<dd>
							<input type="checkbox" name="cuser.userLock" id="userLock"
								<c:if test="${ cuser.userLock == 1}">checked="checked"</c:if>
								value="1" />
							锁定
						</dd>
					</dl>

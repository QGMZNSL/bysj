 ...
 */
public class PlanLevelDbService {
	private DbUtil dbUtil = null;

	/**
	 * 构造
	 */
	public PlanLevelDbService() {
	 ...
	 * @return
	 */
	public List<PlanLevel> qry(PlanLevel planlevel, Page page) {
		List<PlanLevel> planlevels = null;
		StringBuilder sql = new StringBuilder();
		StringBuilder sqlCount = new StringBuilder();
		sql.append("select d.* from base_level d where d.IS_USE='1'");
		sqlCount.append("select count(*) from base_level d where d.IS_USE='1'");
/*		if(!StringTool.isEmpty(planlevel)){
			if(!StringTool.isEmpty(planlevel.getLevelCode())){
				sql.append(String.format(" and level_code like '%%%1$s%%'",planlevel.getLevelCode()));
				sqlCount.append(String.format(" and level_code like '%%%1$s%%'",planlevel.getLevelCode()));
			}
			if(!StringTool.isEmpty(planlevel.getLevelName())){
				sql.append(String.format(" and level_name like '%%%1$s%%'",planlevel.getLevelName()));
				sqlCount.append(String.format(" and level_name like '%%%1$s%%'",planlevel.getLevelName()));
			}
		}*/
	...
	public boolean save(PlanLevel planlevel, int type) {
		...
		switch (type) {
		case 0:
			String s="select count(*) from base_level b where b.level_code=?";
			Long l=dbUtil.getNum(s, planlevel.getLevelCode());
			if(l==0){
				sql.append("insert into base_level");
				sql.append("(level_code,level_name,is_use)");
				sql.append(" values");
				sql.append(String.format(" ('%1$s','%2$s','%3$s')",
					planlevel.getLevelCode(),planlevel.getLevelName(),"1"));//planlevel.getIsUse()
			}
			else{
				sql.append("update base_level");
				sql.append(String.format(" set level_name = '%1$s',",planlevel.getLevelName()));
				sql.append(String.format(" is_use = '%1$s'", "1"));//planlevel.getIsUse()
	...
	public boolean isUseDel(String levelCode){
		boolean delFlag = false;
		StringBuilder sql = new StringBuilder();
		sql.append("update base_level b set b.IS_USE='0'");
		sql.append(String.format(" where b.level_code = '%1$s'",levelCode));
		delFlag = dbUtil.saveOrUpdate(sql.toString()) == 1;
		return delFlag;
	}
}